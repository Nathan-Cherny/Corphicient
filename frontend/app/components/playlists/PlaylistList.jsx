...
export default function PlaylistList({}) {
  ...
  useEffect(() => {
    ...
    if (prevCurrentSong) {
      axiosClient(
        `songs/${prevCurrentSong?.id}/add_time_played/`,
        { amount: (date - currentSongDate) / 1000 },
        null,
        "PUT",
        false,
      );
    ...
    document.title =
      currentSong && !anom
        ? `${currentSong.name} | ${activePlaylist.name} | Corphicient`
        : "Corphicient";
  }, [currentSong]);
      ...
      >
        {playlists.map((pl, i) => (
          <button
            ...
            key={i}
            id={pl.id}
            className={`border bg-blue-50 p-5 hover:scale-105 hover:cursor-pointer ${pl?.id === activePlaylist?.id ? "bg-yellow-500 border-2 font-bold" : ""}`}
          >
            {pl.name}
          </button>
        ))}
...
function changePlaylist(
  ...
  if (activePlaylist && id == activePlaylist.id) {
    return;
  }
  setCurrentSong(null);
