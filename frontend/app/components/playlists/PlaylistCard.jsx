...
export default function PlaylistCard({ playlist, currentSong, setCurrentSong }) {
  return (
    <div className="relative flex flex-col border p-5">
      ...
      <button onClick={() => Playlist.deletePlaylist(playlist.id)} className="absolute top-2 text-black w-6 h-6 flex items-center justify-center hover:scale-110 hover:cursor-pointer transition-all duration-200">
        <Edit/>
      </button>
